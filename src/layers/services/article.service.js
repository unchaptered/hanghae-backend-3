...
const createArticle = async (userId, title, content) => {
    ...
    try {
        ...
        const cretedArticle = await articleRepository.createArticle(
            poolConnection,
            userId,
            title,
            content,
...
const updateArticleById = async (userId, articleId, title, content) => {
    ...
    try {
        ...
        const isUpdated = await articleRepository.updateArticleById(
            poolConnection,
            articleId,
            title,
            content,
...
const updateArticleLike = async (userId, articleId, isLike) => {
    ...
    try {
        ...
        const isArticleLikeExists = await articleRepository.isLikeExists(
            poolConnection,
            userId,
            articleId,
        );
        if (isArticleLikeExists)
            await articleRepository.updateArticleLike(poolConnection, userId, articleId, isLike);
        else {
            await articleRepository.createArticleLike(poolConnection, userId, articleId, isLike);
        }
